...
int main(void)
{
	...
	if (!glfwInit()) //GLFW: 창을 만들고, OpenGL context를 초기화해주는 라이브러리
		return -1;

	/* Create a windowed mode window and its OpenGL context */
	window = glfwCreateWindow(640, 480, "Hello World", NULL, NULL);  //640x480 크기의 윈도우를 생성
	if (!window)
	{
		glfwTerminate();
		return -1;
	...
	glfwMakeContextCurrent(window); //이 윈도우에 OpenGL 명령을 적용할 수 있도록 설정
